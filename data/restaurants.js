export const RESTAURANTS = [
  {
    id: "rest-1",
    name: "Ndole House Douala",
    phone: "+237677123456",
    image: require("../assets/restaurants/photo-1.png"),
    eta: "25-35 min",
    rating: 4.7,
    cuisine: "Cameroonian",
    menu: [
      {
        id: "item-1",
        name: "Ndole with Plantain",
        description: "Classic bitterleaf stew with beef and ripe plantain.",
        price: 3500,
        image: require("../assets/restaurants/photo-1.png"),
      },
      {
        id: "item-2",
        name: "Eru and Water Fufu",
        description: "Rich eru soup with smoked fish and tender beef.",
        price: 4200,
        image:
          "https://images.unsplash.com/photo-1547592166-23ac45744acd?auto=format&fit=crop&w=1000&q=80",
      },
      {
        id: "item-3",
        name: "Pepper Soup",
        description: "Spicy broth with goat meat and local herbs.",
        price: 3200,
        image:
          "https://images.unsplash.com/photo-1547592180-85f173990554?auto=format&fit=crop&w=1000&q=80",
      },
    ],
  },
  {
    id: "rest-2",
    name: "Yassa Corner Yaounde",
    phone: "+237699234567",
    image: require("../assets/restaurants/photo-2.png"),
    eta: "20-30 min",
    rating: 4.5,
    cuisine: "West African Grill",
    menu: [
      {
        id: "item-4",
        name: "Chicken Yassa",
        description: "Caramelized onion and lemon chicken with rice.",
        price: 3900,
        image:
          "https://images.unsplash.com/photo-1600891964092-4316c288032e?auto=format&fit=crop&w=1000&q=80",
      },
      {
        id: "item-5",
        name: "Braised Fish",
        description: "Whole fish with hot pepper sauce and cassava sticks.",
        price: 5000,
        image:
          "https://images.unsplash.com/photo-1544025162-d76694265947?auto=format&fit=crop&w=1000&q=80",
      },
      {
        id: "item-6",
        name: "Jollof Rice Bowl",
        description: "Smoky rice with grilled turkey and veggies.",
        price: 3300,
        image:
          "https://images.unsplash.com/photo-1631515243349-e0cb75fb8d3a?auto=format&fit=crop&w=1000&q=80",
      },
    ],
  },
  {
    id: "rest-3",
    name: "Brioche Fast Bites",
    phone: "+237656345678",
    image: require("../assets/restaurants/photo-3.png"),
    eta: "15-25 min",
    rating: 4.3,
    cuisine: "Street Food",
    menu: [
      {
        id: "item-7",
        name: "Puff-Puff Combo",
        description: "Fresh puff-puff with spicy bean dip.",
        price: 1500,
        image:
          "https://images.unsplash.com/photo-1509440159596-0249088772ff?auto=format&fit=crop&w=1000&q=80",
      },
      {
        id: "item-8",
        name: "Suya Wrap",
        description: "Spiced grilled beef strips in soft flatbread.",
        price: 2400,
        image:
          "https://images.unsplash.com/photo-1520072959219-c595dc870360?auto=format&fit=crop&w=1000&q=80",
      },
      {
        id: "item-9",
        name: "Fried Plantain Pack",
        description: "Golden plantain with mini pepper sauce pot.",
        price: 1800,
        image:
          "https://images.unsplash.com/photo-1604908177522-40256d8d22ee?auto=format&fit=crop&w=1000&q=80",
      },
    ],
  },

  {
    id: "rest-4",
    name: "NI's Kitchen Douala",
    phone: "+237681456789",
    image: require("../assets/restaurants/nis-kitchen.png"),

    eta: "15-25 min",
    rating: 4.3,
    cuisine: "Cameroonian traditional",
    menu: [
      {
        id: "item-7",
        name: "Corn Chaff",
        description: "Fresh corn chaff with spices.",
        price: 1500,
        image:
          "https://images.unsplash.com/photo-1509440159596-0249088772ff?auto=format&fit=crop&w=1000&q=80",
      },
      {
        id: "item-8",
        name: "Tanchop Soup",
        description: "Traditional Cameroonian soup with spices.",
        price: 2400,
        image:
          "https://images.unsplash.com/photo-1520072959219-c595dc870360?auto=format&fit=crop&w=1000&q=80",
      },
      {
        id: "item-9",
        name: "Fried Rice and Chicken",
        description: "Traditional Cameroonian fried rice with chicken.",
        price: 1800,
        image:
          "https://images.unsplash.com/photo-1604908177522-40256d8d22ee?auto=format&fit=crop&w=1000&q=80",
      },
      {
        id: "item-10",
        name: "Eru and Water Fufu",
        description: "Traditional Cameroonian dish with Eru and Water Fufu.",
        price: 1800,
        image:
          "https://images.unsplash.com/photo-1604908177522-40256d8d22ee?auto=format&fit=crop&w=1000&q=80",
      },
    ],
  },
];
